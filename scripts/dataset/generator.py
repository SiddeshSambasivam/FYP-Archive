...
class Generator(object):
    ...
    def __init__(self, params):
        ...
        self.coefficients = [
            f"{x}_{i}"
            for x in self.placeholders.keys()
            for i in range(2 * params.max_len)
        ]
    ...
    def return_rewrite_functions(cls, params):
        r = [x for x in params.rewrite_functions.split(",") if x != ""]
    ...
    def get_leaf(self, curr_leaves, rng):
        ...
        return [
            list(self.variables.keys())[
                rng.randint(low=0, high=min(max_idxs + 1, len(self.variables.keys())))
            ]
        ]
    ...
    def _prefix_to_infix(cls, expr, coefficients=None, variables=None):
        ...
        if t in cls.operators:
            ...
            return "{" + t + "}", expr[1:]
        elif t in variables or t in cls.constants or t == "I":
            return t, expr[1:]
        else:  # INT
            val = expr[0]
    ...
    def infix_to_sympy(cls, infix, variables, rewrite_functions, no_rewrite=False):
        ...
        try:
            expr = parse_expr(
                infix, evaluate=True, local_dict=cls.return_local_dict(variables)
            )
        except ValueError:
    ...
    def process_equation(self, infix):
        ...
        for s in symbols:
            if not len(set(self.var_symbols[: self.pos_dict[s]]) & symbols) == len(
                self.var_symbols[: self.pos_dict[s]]
            ):
                raise NotCorrectIndependentVariables()
