...
def harmonic(x1):
    if all(val.is_integer() for val in x1):
        return np.array(
            [sum(Fraction(1, d) for d in range(1, int(val) + 1)) for val in x1],
            dtype=np.float32,
        )
    else:
        return (
            GAMMA + np.log(x1) + 0.5 / x1 - 1.0 / (12 * x1 ** 2) + 1.0 / (120 * x1 ** 4)
        )
...
protected_ops = [
    ...
    Token(
        protected_log, "logabs", arity=1, complexity=4
    ),  # Protected logabs is support, but redundant
...
def create_tokens(
    ...
    for i in range(n_input_var):
        token = Token(
            name="x{}".format(i + 1), arity=0, complexity=1, function=None, input_var=i
        )
        tokens.append(token)
