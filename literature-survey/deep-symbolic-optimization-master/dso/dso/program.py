...
def _finish_tokens(tokens):
    ...
    if -n_objects in (dangling - 1):
        ...
        assert (
            n_objects == 1
        ), "Is max length constraint turned on? Max length constraint required when n_objects > 1."
        tokens = np.append(
            tokens, np.random.choice(Program.library.input_tokens, size=dangling[-1])
        )
    ...
    if isinstance(str_tokens, list):
        ...
        for s in str_tokens:
            if s in Program.library.names:
                ...
                assert (
                    "const" not in str_tokens
                ), "Currently does not support both placeholder and hard-coded constants."
...
class Program(object):
    ...
    def _init(self, tokens, on_policy=True):
        ...
        self.const_pos = [
            i
            for i, t in enumerate(self.traversal)
            if isinstance(t, PlaceholderConstant)
        ]
        self.len_traversal = len(self.traversal)

        if self.have_cython and self.len_traversal > 1:
            self.is_input_var = array.array(
                "i", [t.input_var is not None for t in self.traversal]
            )
    ...
    def __getstate__(self):
        ...
        state_dict = {
            "tokens": self.tokens,  # string rep comes out different if we cast to array, so we can get cache misses.
            "have_r": bool(have_r),
            ...
            "const": array.array("d", self.get_constants())
            if possible_const
            else float(-np.inf),
            ...
            if not Program.protected:
                ...
                for trav in self.traversals:
                    (
                        ...
                        self.error_node,
                        self.error_type,
                    ) = Program.execute_function(trav, X)
                    result.append(val)
    ...
    def set_constants(self, consts):
        ...
        for i, const in enumerate(consts):
            assert (
                U.is_float
            ), "Input to program constants must be of a floating point type"
    ...
    def set_execute(cls, protected):
        ...
        if protected:
            ...
            class InvalidLog:
                ...
                def __init__(self):
                    self.error_type = (
                        None  # One of ['divide', 'overflow', 'underflow', 'invalid']
                    )
            ...
            np.seterrcall(
                invalid_log
            )  # Tells numpy to call InvalidLog.write() when encountering a warning
    ...
    def evaluate(self):
        ...
        if "r" not in self.__dict__:
            print(
                "WARNING: Evaluating Program before computing its reward."
