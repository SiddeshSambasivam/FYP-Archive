...
def learn(
    ...
    assert (
        n_samples is None or n_epochs is None
    ), "At least one of 'n_samples' or 'n_epochs' must be None."
    ...
    if use_memory:
        assert (
            epsilon is not None and epsilon < 1.0
        ), "Memory queue is only used with risk-seeking."
        memory_queue = make_queue(
            controller=controller, priority=False, capacity=int(memory_capacity)
        )
    ...
    if Program.task.stochastic:
        ...
        assert (
            Program.library.const_token is None
        ), "Constant tokens not yet supported with stochastic Tasks."
        assert (
            not save_pareto_front
        ), "Pareto front not supported with stochastic Tasks."
    else:
        r_history = None
    ...
    for epoch in range(n_epochs):
        ...
        s_history = set(
            r_history.keys() if Program.task.stochastic else Program.cache.keys()
        )
        ...
        if baseline == "ewma_R":
            ewma = (
                np.mean(r_train)
                if ewma is None
                else alpha * np.mean(r_train) + (1 - alpha) * ewma
            )
            b_train = ewma
            ...
            ewma = (
                np.min(r_train)
                if ewma is None
                else alpha * quantile + (1 - alpha) * ewma
            )
            b_train = ewma
        ...
        lengths = np.array(
            [min(len(p.traversal), controller.max_length) for p in p_train],
            dtype=np.int32,
        )
        ...
        if eval_all and any(success):
            print(
                "[{}] Early stopping criteria met; breaking early.".format(
