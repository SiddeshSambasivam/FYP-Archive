...
def make_failed_message(caller, i, n, msg):
    '>>> Test Failed! Caller: {} ({}) {} : TEST {}/{}: "{}" '.format(
        caller.filename, caller.lineno, caller.function, i + 1, n, msg
    )


def make_testing_message(caller, i, n, msg):
    print(
        '>>> Testing Caller: {} ({}) {} : TEST {}/{}: "{}" '.format(
            caller.filename, caller.lineno, caller.function, i + 1, n, msg
        )
    )
...
def assert_invalid(model, cases, n_objects=1):
    ...
    print(batch)
    print(logp)
    assert all(np.isneginf(logp)), "Found invalid case with probability > 0."
...
def assert_valid(model, cases, n_objects=1):
    ...
    batch = make_batch(model, cases, n_objects=n_objects)
    logp = model.controller.compute_probs(batch, log=True)
    assert all(logp > -np.inf), "Found valid case with probability 0."
...
def pre_assert_is_violation(model, cases, prior_class, caller):
    ...
    for i, a in enumerate(batch.actions):
        ...
        r1 = prior_class.is_violated(
            a, parents, siblings
        )  # Tests an optimized version if we have one
        r2 = prior_class.test_is_violated(
            a, parents, siblings
        )  # Tests the slower universal version

        make_testing_message(caller, i, len(batch.actions), "{} == {}".format(r1, r2))

        assert r1 == r2, make_failed_message(
            caller,
            i,
            len(batch.actions),
...
def assert_is_violation_true(model, cases, prior_class):
    ...
    for i, r in enumerate(results):
        assert r, make_failed_message(
            caller, i, len(results), "Return value should be TRUE, but is not."
        )
...
def assert_is_violation_false(model, cases, prior_class):
    ...
    results = pre_assert_is_violation(model, cases, prior_class, caller)
    for i, r in enumerate(results):
        assert not r, make_failed_message(
            caller, i, len(results), "Return value should be FALSE, but is not."
        )
...
def make_batch(model, actions, n_objects=1):
    ...
    actions = np.array(
        [np.pad(a, (0, L - len(a)), "constant") for a in actions], dtype=np.int32
    )
    ...
    for i in range(L):
        ...
        finished = np.where(
            np.logical_and(dangling == 1 - n_objects, lengths == 0), True, False
        )
        lengths = np.where(finished, i + 1, lengths)
...
def test_descendant(model):
    ...
    U = [
        i for i in library.unary_tokens if i not in ancestors and i not in descendants
    ][0]
    B = [
        i for i in library.binary_tokens if i not in ancestors and i not in descendants
    ][0]
...
def test_trig(model):
    ...
    U = [i for i in library.unary_tokens if i not in library.trig_tokens][0]
...
def test_uchild(model):
    ...
    prior_class = RelationalConstraint(
        Program.library, **model.config_prior["relational"]
    )
