...
@click.option(
    "--rollout_length",
    default=10,
    help="How long of episodes to do rollouts for comparing environments.",
)
def setup_envs_run_rollouts(seed: int, rollout_length: int):
    ...
    discrete_envs = {
        "LunarLander-v2": {},
        "LunarLanderCustom-v0": dict(reward_shaping_coef=1, continuous=False),
    }
    ...
    for i, envs in enumerate(all_envs):
        ...
        assert np.array_equal(
            results[names[0]]["state"], results[names[1]]["state"]
        ), "States do not match between above environments."
        assert np.array_equal(
            results[names[0]]["reward"], results[names[1]]["reward"]
        ), "Rewards do not match between above environments."
        assert np.array_equal(
