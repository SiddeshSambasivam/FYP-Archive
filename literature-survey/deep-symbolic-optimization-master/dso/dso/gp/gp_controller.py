...
class GPController:
    def __init__(
        ...
        self.toolbox, self.creator = self._create_toolbox(
            self.pset, tournament_size=tournament_size, mutate_tree_max=mutate_tree_max
        )

        # Actual loop function that runs GP
        self.algorithm = gp_base.RunOneStepAlgorithm(
            toolbox=self.toolbox, cxpb=p_crossover, mutpb=p_mutate, verbose=verbose
        )
    ...
    def _create_toolbox(self, pset, tournament_size=3, mutate_tree_max=5):
        ...
        creator.create(
            "Individual", gp.PrimitiveTree, fitness=creator.FitnessMin
        )  # Adds fitness into PrimitiveTree
    ...
    def __call__(self, actions):
        ...
        individuals = [
            self.creator.Individual(U.tokens_to_DEAP(a, self.pset)) for a in actions
        ]
